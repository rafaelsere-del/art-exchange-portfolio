...
export default function AdminPage({ user, setPage, settings = {}, setSettings }) {
  ...
  const generateInvite = async () => {
    ...
    const newInvite = { id: token, email, token, invitedBy: user.uid, invitedByName: user.name, invitedAt: new Date(), status: "pending", expiresAt };
      ...
      for (const row of rows) {
        ...
        try {
          ...
          await setDoc(doc(db, "users", cred.user.uid), {
            ...
            role: "artist", createdAt: new Date(), createdBy: "admin_import"
          });
  ...
  const artworkRows = users
    .flatMap(u => (u.artworks || []).map(art => ({ ...art, userName: u.name, userLocation: u.location, userUid: u.uid })))
      ...
      <div style={{ padding: "32px 40px" }}>
          ...
          <div style={{ background: "white", borderRadius: RADIUS, overflow: "hidden", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
            {loading ? (
              ...
              <table className="admin-table">
                <thead><tr><th>Name</th><th>Email</th><th>Location</th><th>Role</th><th>Artworks</th><th>Actions</th></tr></thead>
                <tbody>
                  {users.map(u => (
                    <tr key={u.uid}>
                      <td>
                      ...
                      <td>
                        {u.hasArtwork
                          ? <button onClick={() => { setSelectedUser(u); setTab("artworks"); }} style={{ ...btn("#f5f0e8", "#0d0d0d"), fontSize: "0.62rem" }}>
                      ...
                      <td>
                        {u.uid !== user.uid && (
                          <button onClick={() => deleteUser(u.uid, u.name)} style={btn("#c94b2d15", "#c94b2d")}>Delete</button>
                        )}
                      </td>
                    </tr>
                  ))}
          ...
          <div>
            {selectedUser && (
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                <div style={{ fontSize: "0.78rem" }}>Showing artworks by <strong>{selectedUser.name}</strong></div>
                <button onClick={() => setSelectedUser(null)} style={btn("#f5f0e8", "#0d0d0d")}>Show All</button>
              </div>
            )}
            <div style={{ background: "white", borderRadius: RADIUS, overflow: "hidden", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
              {loading ? (
                ...
                <table className="admin-table">
                  <thead><tr><th>Artwork</th><th>Artist</th><th>Location</th><th>Details</th><th>Actions</th></tr></thead>
                  <tbody>
                    {artworkRows.map(art => (
                      <tr key={art.id}>
                        <td>
                        ...
                        <td>
                          <button onClick={() => deleteArtwork(art.userUid, art.id, art.title)} style={btn("#c94b2d15", "#c94b2d")}>Delete</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        {tab === "import" && (
          <div style={{ maxWidth: 640 }}>
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "1.6rem", fontWeight: 900, marginBottom: 8 }}>Import Users from Excel</h2>
            ...
            <div onClick={() => fileRef.current.click()}
              ...
              {importing
                ? <><div style={{ fontSize: "1.5rem", marginBottom: 10 }}>⏳</div><div style={{ fontSize: "0.73rem", color: "#9e9589" }}>Creating users...</div></>
                : <><div style={{ fontSize: "1.8rem", marginBottom: 10 }}>📊</div><div style={{ fontSize: "0.73rem", color: "#9e9589", textAlign: "center", lineHeight: 1.7 }}>Drop your .xlsx file here<br /><span style={{ color: "#c94b2d", textDecoration: "underline" }}>or click to browse</span></div></>
              }
              <input ref={fileRef} type="file" accept=".xlsx,.xls" style={{ display: "none" }} onChange={e => handleExcel(e.target.files[0])} />
            </div>
            {importResults && (
              <div style={{ marginTop: 28 }}>
                {importResults.created.length > 0 && (
                  <div style={{ background: "#5a7a5e15", border: "1px solid #5a7a5e30", borderRadius: RADIUS, padding: "16px 20px", marginBottom: 16 }}>
                    <div style={{ fontSize: "0.68rem", textTransform: "uppercase", letterSpacing: "0.1em", color: "#5a7a5e", marginBottom: 10, fontWeight: 700 }}>✓ {importResults.created.length} user{importResults.created.length !== 1 ? "s" : ""} created</div>
                    {importResults.created.map((u, i) => <div key={i} style={{ fontSize: "0.72rem", marginBottom: 4 }}>{u.name} — {u.email}</div>)}
                  </div>
                )}
                {importResults.failed.length > 0 && (
                  <div style={{ background: "#c94b2d15", border: "1px solid #c94b2d30", borderRadius: RADIUS, padding: "16px 20px" }}>
                    <div style={{ fontSize: "0.68rem", textTransform: "uppercase", letterSpacing: "0.1em", color: "#c94b2d", marginBottom: 10, fontWeight: 700 }}>✕ {importResults.failed.length} failed</div>
                    {importResults.failed.map((u, i) => <div key={i} style={{ fontSize: "0.72rem", marginBottom: 4 }}>{u.email} — <span style={{ color: "#c94b2d" }}>{u.reason}</span></div>)}
                  </div>
                )}
              </div>
            )}
        ...
        {tab === "applications" && (
          <div style={{ background: "white", borderRadius: RADIUS, overflow: "hidden", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
            {applications.length === 0 ? (
              ...
              <table className="admin-table">
                <thead><tr><th>Applicant</th><th>Email</th><th>Applied</th><th>Status</th><th>Artwork</th><th>Actions</th></tr></thead>
                <tbody>
                  {applications.map(app => (
                    <>
                      <tr key={app.id}>
                        ...
                        <td style={{ color: "#9e9589", fontSize: "0.7rem" }}>{app.createdAt?.toDate?.()?.toLocaleDateString() || "—"}</td>
          ...
          <div>
            <div style={{ background: "white", borderRadius: RADIUS, padding: "24px 28px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", marginBottom: 24 }}>
              ...
              <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                <div style={{ flex: 1 }}>
                  <input value={inviteEmail} onChange={e => { setInviteEmail(e.target.value); setInviteError(""); }} placeholder="artist@example.com"
                  ...
                  {inviteError && <div style={{ fontSize: "0.65rem", color: "#c94b2d", marginTop: 6 }}>{inviteError}</div>}
                </div>
                <button onClick={generateInvite} style={btn("#0d0d0d","#f5f0e8", { padding: "10px 20px" })}>Generate Invite</button>
              </div>
            </div>
            <div style={{ background: "white", borderRadius: RADIUS, overflow: "hidden", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
              {invitations.length === 0 ? (
                ...
                <table className="admin-table">
                  <thead><tr><th>Email</th><th>Invited By</th><th>Sent</th><th>Expires</th><th>Status</th><th>Actions</th></tr></thead>
                  <tbody>
                    {invitations.map(inv => {
                      ...
                      return (
                        <tr key={inv.id} style={{ opacity: dimmed ? 0.45 : 1 }}>
                          ...
                          <td style={{ color: "#9e9589" }}>{inv.invitedByName}</td>
                          <td style={{ color: "#9e9589", fontSize: "0.7rem" }}>{inv.invitedAt?.toDate?.()?.toLocaleDateString() || "—"}</td>
                          <td style={{ color: "#9e9589", fontSize: "0.7rem" }}>{inv.expiresAt?.toDate?.()?.toLocaleDateString() || "—"}</td>
        ...
        {tab === "settings" && (
          <div style={{ maxWidth: 540 }}>
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "1.6rem", fontWeight: 900, marginBottom: 8 }}>Registration Settings</h2>
            <p style={{ fontSize: "0.75rem", color: "#9e9589", lineHeight: 1.8, marginBottom: 32 }}>
            ...
            {[
              ...
              <div key={key} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "20px 0", borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
                ...
                <div onClick={() => setDraftSettings(s => ({ ...s, [key]: !s[key] }))}
            ...
            <div style={{ marginTop: 28, display: "flex", alignItems: "center", gap: 16 }}>
              <button onClick={saveSettings} style={btn("#0d0d0d","#f5f0e8", { padding: "11px 24px" })}>Save Settings</button>
