...
async function seedUser(index) {
  ...
  const artworkBase64 = imageToBase64(artworkPath);
  const sizeKB = Math.round(Buffer.byteLength(artworkBase64) / 1024);

  // 3. Write Firestore profile with base64 image
  ...
  console.log(`✅ ${num.toString().padStart(2,"0")} | ${user.name.padEnd(26)} | ${artworkFile} | ${sizeKB} KB`);
}

async function main() {
  console.log("🎨 ARTxART Firebase Seeder (base64 mode)\n");
  let ok = 0, fail = 0;
